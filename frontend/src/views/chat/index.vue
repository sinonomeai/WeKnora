    ...
    <div class="chat">
        <div ref="scrollContainer" class="chat_scroll_box" @scroll="handleScroll">
            <div class="msg_list">
                ...
                <div
                    ...
                    :class="{ 'has-questions': suggestedQuestions.length > 0 }">
        ...
        <div style="min-height: 115px; margin: 16px auto 4px; width: 100%; max-width: 800px">
            <InputField
                ref="inputFieldRef"
                @send-msg="
                    (query, modelId, mentionedItems, imageFiles) =>
                        sendMsg(query, modelId, mentionedItems, imageFiles)
                "
...
const { t } = useI18n()
const { menuArr, isFirstSession, firstQuery, firstMentionedItems, firstModelId, firstImageFiles } =
...
watch(() => useSettingsStoreInstance.selectedAgentId, debouncedFetchSuggestions)
watch(() => useSettingsStoreInstance.settings.selectedKnowledgeBases, debouncedFetchSuggestions, {
    deep: true,
})
watch(() => useSettingsStoreInstance.settings.selectedFiles, debouncedFetchSuggestions, {
    deep: true,
})
...
const handleMsgList = async (data, isScrollType = false, newScrollHeight) => {
    ...
    for (let i = 0, len = chatlist.length; i < len; i++) {
        ...
        if (item.agent_steps && Array.isArray(item.agent_steps) && item.agent_steps.length > 0) {
            console.log(
                "[Message Load] Reconstructing agent steps for message:",
                item.id,
                "steps:",
            ...
            console.log(
                "[Message Load] Reconstructed",
                item.agentEventStream.length,
                "events from agent steps",
            )
        }
...
const sendMsg = async (value, modelId = "", mentionedItems = [], imageFiles = []) => {
    ...
    // Convert files to base64 data URIs for local image preview and upload non-image attachments to backend
    let imageAttachments = []
    let userImages = []
    let localNonImageAttachments = []
        ...
        try {
            for (const file of imageFiles) {
                ...
                if (file.type && file.type.startsWith("image/")) {
                    ...
                    // Keep local metadata while we upload to backend
                    localNonImageAttachments.push({
                        file,
                        name: file.name,
                        size: file.size,
                        type: file.type || "",
            ...
            console.error("[File] Failed to read files:", e)
            loading.value = false
            isReplying.value = false
            return
        }
    ...
    // Upload non-image attachments to backend and collect returned URLs/ids
    const uploadedAttachments = []
    if (localNonImageAttachments.length > 0) {
        for (const la of localNonImageAttachments) {
            try {
                const formData = new FormData()
                formData.append("file", la.file)
                // If backend needs additional fields (e.g., tenant, tag), add them here
                const res = await postUpload("/api/v1/documents/upload", formData)
                // Try common response shapes: { success, data: { url, id } } or { url, id }
                let info = null
                if (res) {
                ...
                uploadedAttachments.push({
                    ...
                    size: la.size,
                    type: la.type,
                    url: info?.url || info?.path || info?.data?.url || "",
                    id: info?.id || info?.file_id || null,
                })
            } catch (err) {
                console.warn("[Attachment Upload] failed for", la.name, err)
                // Fallback: include local metadata without url so user still sees it
                uploadedAttachments.push({
                    name: la.name,
                    size: la.size,
                    type: la.type,
    ...
    // 将@提及的知识库和文件信息存入用户消息（包含上传后的附件元数据）
    const userMessage = {
        content: value,
        role: "user",
    ...
    if (uploadedAttachments.length > 0) userMessage.attachments = uploadedAttachments
...
onChunk((data) => {
    ...
    if (data.response_type === "agent_query") {
        ...
        // 检查是否是继续流式传输（消息已存在）
        const existingMessage = messagesList.findLast(
    ...
    if (data.response_type === "references") {
        ...
        let existingMessage = messagesList.findLast(
...
const handleAgentChunk = (data) => {
    let message = messagesList.findLast(
        (item) => item.request_id === data.id || item.id === data.id,
    )
