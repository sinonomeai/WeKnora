...
// Attachment upload state (files other than images)
const uploadedAttachments = ref<Array<{ file: File; name: string }>>([])
...
const addAttachmentFiles = (files: File[]) => {
    // Allow up to 10 attachments by default
    const maxAttachments = 10
    for (const file of files) {
        if (uploadedAttachments.value.length >= maxAttachments) {
            MessagePlugin.warning(t("chat.attachmentTooMany") || "Too many attachments")
            break
        }
        // No strict type/size check here; agents may control supported types elsewhere
        uploadedAttachments.value.push({ file, name: file.name })
    }
}
...
const selectedAgent = computed(() => {
    ...
    if (sourceTenantId && orgStore.sharedAgents?.length) {
        const shared = orgStore.sharedAgents.find(
            (s) =>
                s.agent.id === selectedAgentId.value &&
                String(s.source_tenant_id) === sourceTenantId,
        )
...
const selectedKbs = computed(() => {
    ...
    const sharedMapped = sharedList
        .filter(
            (s: any) =>
                s.knowledge_base != null && selectedKbIds.value.includes(s.knowledge_base.id),
        )
        .map((s: any) => ({
    ...
    const sharedFromAgent = (sharedAgentKbList.value || [])
        .filter(
            (kb) =>
                selectedKbIds.value.includes(kb.id) &&
                !ownIds.has(kb.id) &&
                !sharedOnlyIds.has(kb.id),
        )
        .map((kb) => ({
            id: kb.id,
            name: kb.name,
    ...
    try {
        ...
        if (response.data && Array.isArray(response.data)) {
            ...
            const validSelectedIds = currentSelectedIds.filter(
                (id: string) =>
                    validKbIds.has(id) || sharedKbIds.has(id) || sharedAgentKbIdSet.has(id),
            )
...
const loadAgents = async () => {
    try {
        const [agentsRes] = await Promise.all([listAgents(), orgStore.fetchSharedAgents()])
        ...
        try {
            ...
            if (res.data && Array.isArray(res.data)) {
                ...
                if (
                    ...
                    agentKBSelectionMode.value === "selected"
                ) {
                    const configuredKbIds = agentKnowledgeBases.value
                    files = files.filter((f: any) =>
                        configuredKbIds.includes(f.knowledge_base_id ?? f.kb_id),
                    )
                }
                const sharedKbOrgMap: Record<string, string> = {}
                ;(orgStore.sharedKnowledgeBases || []).forEach((s: any) => {
                ...
                fileItems = files.map((f: any) => {
                    ...
                    const fileOrgName =
                        agentOrgLabel || (kbIdStr ? sharedKbOrgMap[kbIdStr] : undefined)
                    return {
                        id: f.id,
...
const createSession = async (val: string) => {
    ...
    const attachmentFiles = uploadedAttachments.value.map((a) => a.file)
    // 合并图片与附件，保留原来接收 imageFiles 的接口（将附件并入其中）
    const filesToSend = [...imageFiles, ...attachmentFiles]
    ...
    emit("send-msg", val, selectedModelId.value, mentionedItems, filesToSend)
...
<template>
    <div class="answers-input" @drop="onDrop" @dragover="onDragOver">
        ...
        <div class="rich-input-container">
            ...
            <!-- 附件预览区域 -->
            <div v-if="uploadedAttachments.length > 0" class="attachment-preview-bar">
                <div
                    v-for="(att, idx) in uploadedAttachments"
                    :key="idx"
                    class="attachment-preview-item">
                    <span class="attachment-name" :title="att.name">{{ att.name }}</span>
                    <span class="attachment-remove" @click="removeAttachment(idx)">×</span>
                </div>
            </div>
                        ...
                        <span v-if="item.org_name" class="mention-chip__org-badge">
                            <img
                                :src="
                                    getImgSrc(
                                        item.type === 'file'
                                            ? 'organization-grey.svg'
                                            : 'organization-green.svg',
                                    )
                                "
        ...
        <div class="control-bar">
            ...
            <div class="control-left">
                ...
                <t-tooltip
                    ...
                    <template #content>
                        <div v-if="isWebSearchDisabledByAgent" class="tooltip-with-link">
                            ...
                            <a href="#" @click.prevent="handleGoToAgentSettings('websearch')">{{
                ...
                <t-tooltip
                    ...
                    <template #content>
                        <div v-if="!isImageUploadEnabledByAgent" class="tooltip-with-link">
                            ...
                            <a href="#" @click.prevent="handleGoToAgentSettings('model')">{{
                                $t("input.goToAgentSettings")
                            }}</a>
                        </div>
                ...
                <t-tooltip
                    ...
                    <template #content>
                        <span>{{
                            $t("input.attachmentUpload") || $t("input.attachment") || "Attachment"
                        }}</span>
                    </template>
                    <div
                        ...
                        @click.stop="triggerAttachmentUpload()">
                        <svg
                            width="18"
                            height="18"
                            viewBox="0 0 24 24"
                            ...
                            class="control-icon">
                            <path
                                d="M21.44 11.05l-8.49 8.49a5 5 0 0 1-7.07-7.07l7.07-7.07a3.5 3.5 0 1 1 4.95 4.95l-6.36 6.36a2 2 0 0 1-2.83-2.83l5.66-5.66"
                                stroke="currentColor"
                                stroke-width="1.2"
                                stroke-linecap="round"
...
.selected-tags-inline {
    ...
    border-bottom: 0.5px solid var(--td-component-stroke, #e7e7e7);
    background: var(--td-bg-color-container, #fff);
...
/* Attachment upload button (match hover/active behavior with image and websearch) */
...
.model-selector-dropdown {
    ...
    border: 0.5px solid var(--td-component-border, #e7e9eb);
    overflow: hidden;
    display: flex;
    flex-direction: column;
    margin: 0 !important;
...
.model-selector-header {
    ...
    justify-content: space-between;
    padding: 8px 12px;
    border-bottom: 0.5px solid var(--td-component-stroke, #f0f0f0);
    background: var(--td-bg-color-container, #fff);
    font-size: 12px;
    font-weight: 500;
    color: var(--td-text-color-secondary, #666);
