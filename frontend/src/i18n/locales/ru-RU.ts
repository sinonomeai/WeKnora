export default {
    ...
    agent: {
        ...
        stepsCompletedWithDuration:
            "Выполнено <strong>{steps}</strong> шаг(ов) за <strong>{duration}</strong>",
        editor: {
            skillsConfig: "Skills",
            skillsConfigDesc:
    ...
    settings: {
        ...
        enableMemoryDesc:
            "При включении система будет записывать историю ваших разговоров и автоматически вспоминать соответствующий контент в будущих беседах для более персонализированных ответов.",
        memoryRequiresNeo4j:
            "Функция памяти требует графовую базу данных Neo4j. Пожалуйста, настройте и включите Neo4j (установите NEO4J_ENABLE=true) перед активацией этой функции.",
    ...
    webSearchSettings: {
        ...
        noProviders:
            "Поисковые провайдеры не настроены. Нажмите «Добавить провайдер», чтобы начать.",
        ...
        defaultProviderDescription:
            "Выберите провайдер поиска по умолчанию для агентов, не указавших собственный.",
...
/example\.(net|org)/`,
    ...
    retrievalSettings: {
        ...
        vectorThresholdDescription:
            "Минимальная оценка сходства для векторного поиска (0-1, выше — точнее)",
        keywordThresholdLabel: "Порог совпадения ключевых слов",
        keywordThresholdDescription:
            "Минимальная оценка совпадения для поиска по ключевым словам (0-1)",
        ...
        rerankThresholdDescription:
            "Минимальный порог оценки для повторного ранжирования (от -10 до 10)",
    ...
    knowledgeEditor: {
        ...
        chunking: {
            ...
            parentChildDescription:
                "Включить двухуровневую стратегию разбиения. Большие родительские блоки обеспечивают контекст, а маленькие дочерние блоки используются для векторного поиска.",
        ...
        asr: {
            ...
            description:
                "Настройте распознавание речи для загрузки аудиофайлов (mp3, wav, m4a, flac, ogg) и автоматической транскрибации в текст",
    ...
    agentSettings: {
        ...
        globalConfigNotice:
            "Это глобальные настройки по умолчанию. Новые агенты унаследуют эти настройки. Вы также можете настроить каждого агента индивидуально в списке агентов.",
        ...
        systemPrompt: {
            ...
            tabHintDetail:
                "Единый системный промпт (оставьте пустым для значения по умолчанию, используйте {'{{'}web_search_status{'}}'} для динамического управления веб-поиском)",
    ...
    conversationSettings: {
        ...
        contextTemplate: {
            ...
            descWithDefault:
                "Шаблон промпта для генерации ответов на основе результатов поиска в обычном режиме (оставьте пустым для значения по умолчанию)",
            placeholder: "Введите шаблон промпта для суммирования результатов поиска...",
            custom: "Пользовательский шаблон",
            disabledHint:
                "Сейчас используется промпт по умолчанию. Включите пользовательский для редактирования.",
        },
        systemPrompt: {
            label: "Системный промпт",
            desc: "Системный промпт для обычного режима диалога",
            descWithDefault:
                "Системный промпт для обычного режима диалога (оставьте пустым для значения по умолчанию)",
    ...
    modelSettings: {
        ...
        builtinModels: {
            ...
            description:
                "Встроенные модели видны всем тенантам. Конфиденциальная информация скрыта, их нельзя редактировать или удалять.",
    ...
    input: {
        ...
        attachmentUpload: "Загрузить файл",
        attachment: "Вложение",
        knowledgeBase: "База знаний",
    ...
    knowledgeSearch: {
        title: "Поиск",
        subtitle:
            "Семантический поиск по базам знаний и истории сообщений для быстрого нахождения релевантного контента",
    ...
    kbSettings: {
        storage: {
            title: "Хранилище",
            description:
                "Выберите хранилище файлов. Это влияет на способ хранения загруженных документов и изображений в документах. Параметры настраиваются в глобальных настройках.",
            loading: "Загрузка...",
            engineLabel: "Хранилище",
            engineDesc:
                "Выберите хранилище для этой базы знаний. Соответствующее хранилище должно быть настроено в глобальных настройках.",
            selectPlaceholder: "Выберите хранилище",
            notConfigured: "Не настроено",
            unavailable: "Недоступно",
            lockedHint:
                "В базе знаний уже есть файлы. Невозможно сменить хранилище. Для смены сначала удалите все файлы.",
            changeWarning:
                "Смена хранилища влияет только на новые загружаемые файлы. Существующие файлы по-прежнему будут читаться из прежнего хранилища, но некоторые старые файлы могут стать недоступными.",
    ...
    agentEditor: {
        ...
        im: {
            title: "Интеграция IM",
            description:
                "Подключите агента к платформам мгновенных сообщений, таким как Feishu, Slack, Telegram, DingTalk и Mattermost",
            feishu: "Feishu",
            slack: "Slack",
            telegram: "Telegram",
            dingtalk: "DingTalk",
        ...
        imageUpload: {
            ...
            sectionDesc:
                "Настройте загрузку изображений и визуально-языковую модель для мультимодальных диалогов",
        ...
        audioUpload: {
            ...
            desc: "Позволяет пользователям загружать аудиофайлы для распознавания или транскрипции речи в диалоге (mp3, wav, m4a, flac, ogg и др.)",
            asrModel: "ASR-модель",
            asrModelDesc:
                "Модель распознавания речи для преобразования аудио в текст (например, OpenAI Whisper)",
            asrModelPlaceholder: "Выберите ASR-модель",
        },
        faq: {
            title: "Стратегия приоритета FAQ",
            tooltip:
                "Если база знаний содержит FAQ (пары вопрос-ответ), включите эту стратегию для приоритета ответов FAQ над обычными документами",
    ...
    datasource: {
        ...
        permissionHint:
            "Приложение должно быть добавлено как участник пространства вики для доступа к контенту:",
